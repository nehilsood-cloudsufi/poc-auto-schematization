...
python src/run_pipeline.py --dataset=bis_bis_central_bank_policy_rate \
  --ground-truth-pvmap=ground_truth/bis_bis_central_bank_policy_rate/pvmap/pvmap.csv
...
PYTHONPATH="$(pwd):$(pwd)/src" python3 tools/stat_var_processor.py \
  ...
  --config_file="input/dataset_name/input_metadata/metadata.csv" \
  --generate_statvar_name=True \
  --output_path="output/dataset_name/processed"
# Note: --config_file is optional. Omit if no metadata is available.
...
- Discovers input data (from `test_data/`), optional metadata (from `input_metadata/`), schema files (from `schema/`), and ground truth metadata
...
- Copies appropriate schema example files (.txt and .mcf) to `schema/` subdirectory
...
5. **File path patterns are strict:**
   - Metadata: `input_metadata/*_metadata.csv`
   - Sampled data: `test_data/*_sampled_data.csv`
   - Input data: `test_data/*_input.csv`
   - Schema examples: `schema/scripts_*_schema_examples_*.txt`
   - Schema MCF: `schema/scripts_*_vertical_*.mcf`
...
# Note: --config_file is optional. Omit if dataset has no metadata.
```
...
- `--metadata-file-path` - Explicit metadata file (auto-enables `--use-metadata`)
- `--schema-file` - Explicit schema file override
