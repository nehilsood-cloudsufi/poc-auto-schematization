"""File management for UI runs with output versioning."""
import json
import logging
import shutil
...
from src.ui.config import UI_OUTPUT_DIR
...
def snapshot_version(output_dir: Path, version: int) -> Path:
    ...
    copied = 0
    # Copy all output files to version dir
...
def discover_historical_runs() -> list[dict]:
    """Scan ui_output/ for past runs and return summary metadata.
    ...
    """
    if not UI_OUTPUT_DIR.exists():
        return []

    runs = []
    for run_dir in UI_OUTPUT_DIR.iterdir():
        ...
        # Fall back to directory mtime if no attempt data
        if not timestamp:
...
def cleanup_old_runs(max_age_hours: int = 24) -> None:
