...
Usage:
    python src/pipeline/schema_selection/schema_selector.py --input_dir=input/dataset_name/
    python src/pipeline/schema_selection/schema_selector.py --input_dir=input/dataset_name/ --dry_run
    python src/pipeline/schema_selection/schema_selector.py --input_dir=input/dataset_name/ --force
"""
...
flags.DEFINE_string('schema_base_dir', None,
                    'Path to schema files directory (default: src/resources/schema_examples/).')
...
DEFAULT_SCHEMA_DIR = BASE_DIR / "src" / "resources" / "schema_examples"
...
def select_schema_for_directory(input_dir: Path,
                                 ...
                                 force: bool = False,
                                 dry_run: bool = False,
                                 preview_rows: int = 15) -> Optional[str]:
    """Programmatic interface for schema selection.

    Args:
        input_dir: Path to the input directory
        schema_base_dir: Path to schema files directory (default: src/resources/schema_examples/)
        gemini_client: Optional GeminiClient instance to use
        force: Force re-selection even if schema files already exist
        dry_run: If True, only show what would be copied
        preview_rows: Number of data rows to include in preview

    Returns:
        Selected category name, or None if selection failed
    """
    ...
    # Step 4: Generate data preview
    data_preview = generate_data_preview(input_dir, preview_rows)

    # Step 5: Get category information and schema previews
    category_info = get_category_info()
    schema_previews = generate_schema_previews(schema_base_dir)

    # Step 6: Build prompt
    prompt = build_prompt(metadata_content, data_preview, category_info, schema_previews)

    # Step 7: Invoke Gemini API
    ...
    # Step 8: Copy schema files
    success, copied_files = copy_schema_files(category, schema_base_dir, input_dir, dry_run)

    # Step 9: Clean up temporary combined metadata file
    if combined_metadata_path and combined_metadata_path.exists():
        combined_metadata_path.unlink()
...
def main(argv):
    ...
    category = select_schema_for_directory(
        input_dir=input_dir,
        schema_base_dir=schema_base_dir,
        force=FLAGS.force,
        dry_run=FLAGS.dry_run,
        preview_rows=FLAGS.preview_rows
    )
