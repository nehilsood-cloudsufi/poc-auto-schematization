...
# Add paths for src/ structure only (util/ and tools/ have been removed)
...
sys.path.append(os.path.join(_PROJECT_ROOT, 'tools', 'place'))
sys.path.append(os.path.join(_PROJECT_ROOT, 'tools', 'schema'))

# Try new import paths first, fall back to old paths
try:
    from src.processing.evaluation import eval_functions
except ImportError:
    import eval_functions

try:
    from src.infrastructure.log.log_util import configure_cloud_logging, running_on_cloud
except ImportError:
    from log_util import configure_cloud_logging, running_on_cloud

try:
    from src.processing.utils import (capitalize_first_char, is_place_dcid,
...
try:
    from src.infrastructure.config import config_flags
except ImportError:
    import config_flags

# data_annotator is optional and has complex dependencies - skip if any error
try:
    from src.data_commons.schema import data_annotator
except Exception:
    data_annotator = None

try:
    from src.processing.mapping import property_value_utils as pv_utils
except ImportError:
    import property_value_utils as pv_utils

try:
    from src.processing.filtering.filter_data_outliers import filter_data_svobs
except ImportError:
    try:
        from filter_data_outliers import filter_data_svobs
    except ImportError:
        filter_data_svobs = None

try:
    from src.data_commons.mcf.mcf_file_util import get_numeric_value, get_value_list, add_pv_to_node
    from src.data_commons.mcf.mcf_file_util import load_mcf_nodes, write_mcf_nodes, add_namespace, strip_namespace
except ImportError:
...
try:
    from src.data_commons.mcf.mcf_filter import drop_existing_mcf_nodes
except ImportError:
    try:
        from mcf_filter import drop_existing_mcf_nodes
    except ImportError:
        drop_existing_mcf_nodes = None

try:
    from src.data_commons.mcf.mcf_diff import fingerprint_node, fingerprint_mcf_nodes, diff_mcf_node_pvs
except ImportError:
    from mcf_diff import fingerprint_node, fingerprint_mcf_nodes, diff_mcf_node_pvs

try:
    from src.data_commons.place.place_resolver import PlaceResolver
except ImportError:
...
try:
    from src.data_commons.schema.schema_resolver import SchemaResolver
except ImportError:
    from schema_resolver import SchemaResolver

try:
    from src.processing.transformation.json_to_csv import file_json_to_csv
except ImportError:
    from json_to_csv import file_json_to_csv

try:
    from src.data_commons.schema.schema_generator import generate_schema_nodes, generate_statvar_name
except ImportError:
    from schema_generator import generate_schema_nodes, generate_statvar_name

try:
    from src.data_commons.schema.schema_checker import sanity_check_nodes
except ImportError:
    from schema_checker import sanity_check_nodes

# imports from util (try new paths first)
try:
    from src.infrastructure.config.config_map import ConfigMap, read_py_dict_from_file
except ImportError:
...
try:
    from src.infrastructure.io.download_util import download_file_from_url
except ImportError:
...
try:
    from src.pipeline.validation import process_http_server
except ImportError:
    process_http_server = None  # Optional, may not be needed
